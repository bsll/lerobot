...
"""Auto calibration helpers for Feetech-based devices (ported from JoyandAI/lerobot)."""
...
class AutoCalibrateConfig:
    """Auto calibration configuration (torque / exploration parameters)."""
...
def auto_calibrate_connected_device(
    ...
    *,
    ...
    # Keep the in-memory device calibration in sync with what was written to the bus.
    if hasattr(device, "calibration"):
        device.calibration = calibration_dict

    if save:
        if calibration_path is not None:
            saved_path = Path(calibration_path)
            save_calibration_to_file(calibration_dict, saved_path)
        elif hasattr(device, "_save_calibration"):
            device._save_calibration()
            saved_path = Path(device.calibration_fpath)
            logger.info("Calibration data saved to: %s", saved_path)
        else:
            saved_path = Path(device.calibration_fpath)
            save_calibration_to_file(calibration_dict, saved_path)
